...
from grid_topology_ai.self_play.pool_sampling import compute_priority, refresh_priorities
...
def update_pool_metadata(
    ...
    *,
    ...
    selected_ids = {int(value) for value in selected_scenario_ids or ()}

    if df.empty:
        for scenario_id in sorted(selected_ids):
            meta = scenarios.get(str(int(scenario_id)))
            if meta is None:
                continue
            meta["times_attempted"] = int(meta.get("times_attempted", 0)) + 1
            meta["last_attempted_iter"] = int(current_iter)
        refresh_priorities(pool_metadata, current_iter=current_iter)
    ...
    for scenario_id in sorted(selected_ids - attempted_ids):
        meta = scenarios.get(str(int(scenario_id)))
        if meta is None:
            continue
        meta["times_attempted"] = int(meta.get("times_attempted", 0)) + 1
