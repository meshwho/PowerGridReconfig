...
def _coerce_integer_scenario_id(value: object) -> int:
    ...
    try:
        ...
        raise ValueError(
            f"scenario_id cannot be converted to an integer: {value!r}."
        ) from exc
...
def _resolved_self_play_config(output_dir: Path) -> SelfPlayConfig | None:
...
def run_train(
    *,
    ...
    if (
        ...
        and not bool(config.save_multiple_best)
    ):
        raise RuntimeError(
    ...
    print(f"  {selection.metric_name}: {selection.metric_value:.6f}")
