...
def validate_ppc_input(ppc: dict[str, Any], physics_config: PhysicsConfig, *, context: str = "ppc") -> None:
    ...
    if ppc.get("version") not in {"2", 2}:
        raise InvalidPhysicalState(f"{context}: unsupported MATPOWER version.")
    try:
        base_mva = float(ppc["baseMVA"])
    except (KeyError, TypeError, ValueError) as exc:
        raise InvalidPhysicalState(f"{context}: baseMVA must be present and numeric.") from exc
    if not np.isfinite(base_mva) or base_mva <= 0.0:
        raise InvalidPhysicalState(f"{context}: baseMVA must be finite and positive.")
    if not np.isclose(base_mva, physics_config.base_mva, rtol=0.0, atol=0.0):
        raise InvalidPhysicalState(f"{context}: baseMVA disagrees with PhysicsConfig.")
    ...
    if np.any(branch[active_branch, ANGMIN] > branch[active_branch, ANGMAX]):
        raise InvalidPhysicalState(f"{context}: active branch has ANGMIN > ANGMAX.")
    if not np.isin(branch[:, BR_STATUS], (0.0, 1.0)).all():
        raise InvalidPhysicalState(f"{context}: branch status must be 0 or 1.")
    if not np.isin(gen[:, GEN_STATUS], (0.0, 1.0)).all():
        raise InvalidPhysicalState(f"{context}: generator status must be 0 or 1.")
