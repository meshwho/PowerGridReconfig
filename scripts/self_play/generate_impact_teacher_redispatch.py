...
def _search_workload(
    ...
    return {
        ...
        "cache_hit_rate": (
            float(cache_hits) / float(cache_lookups) if cache_lookups > 0 else 0.0
        ),
        ...
        "warm_start_rate": (
            float(warm_start_hits) / float(cache_misses) if cache_misses > 0 else 0.0
        ),
        "stock_runpf_calls": int(stock_runpf_calls),
        "q_limit_resolves": int(q_limit_resolves),
        "solves_per_cache_miss": (
            float(stock_runpf_calls) / float(cache_misses) if cache_misses > 0 else 0.0
        ),
    }
...
def _print_power_flow_workload_summary() -> None:
    ...
    logical_evaluations = sum(int(item.get("logical_evaluations", 0)) for item in items)
    cache_hits = sum(int(item.get("cache_hits", 0)) for item in items)
    cache_misses = sum(int(item.get("cache_misses", 0)) for item in items)
    exact_cache_hits = sum(int(item.get("exact_cache_hits", 0)) for item in items)
    tolerant_cache_hits = sum(int(item.get("tolerant_cache_hits", 0)) for item in items)
    warm_start_hits = sum(int(item.get("warm_start_hits", 0)) for item in items)
    cold_start_misses = sum(int(item.get("cold_start_misses", 0)) for item in items)
    stock_runpf_calls = sum(int(item.get("stock_runpf_calls", 0)) for item in items)
    q_limit_resolves = sum(int(item.get("q_limit_resolves", 0)) for item in items)
    cache_lookups = cache_hits + cache_misses
    cache_hit_rate = float(cache_hits) / float(cache_lookups) if cache_lookups else 0.0
    warm_start_rate = float(warm_start_hits) / float(cache_misses) if cache_misses else 0.0
    solves_per_cache_miss = (
        float(stock_runpf_calls) / float(cache_misses) if cache_misses else 0.0
    )
...
_base_init_worker_context = init_worker_context
_base_run_parallel = run_parallel
_provenance_main = main

# Transitional self-aliases used by the staged/runtime layers until Commit 11.
base = teacher
...
def _worker_init_concurrency() -> int:
    raw_value = os.environ.get(_WORKER_INIT_CONCURRENCY_ENV, "1").strip()
    try:
...
def init_worker_context(
    ...
    init_semaphore = runtime_task_config.pop(_RUNTIME_WORKER_INIT_SEMAPHORE, None)
    if init_semaphore is None:
        _base_init_worker_context(
            raw_dir_str,
            states_dir_str,
            runtime_task_config,
            scenario_ids,
