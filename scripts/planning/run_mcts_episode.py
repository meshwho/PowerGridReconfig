...
def main() -> None:
    ...
    for step in range(args.max_steps):
        ...
        print(
            "MCTS coverage: "
            f"considered={search_result.root_considered_action_count}/"
            f"{search_result.root_legal_action_count} "
            f"({search_result.root_action_coverage:.1%}), "
            f"visited={search_result.root_visited_action_count}/"
            f"{search_result.root_legal_action_count} "
        ...
        step_records.append(
            {
                ...
                "considered_action_count": (
                    search_result.root_considered_action_count
                ),
                "legal_action_count": search_result.root_legal_action_count,
                "visited_action_count": search_result.root_visited_action_count,
                "action_coverage": search_result.root_action_coverage,
    ...
    for record in step_records:
        print(
            ...
            f"coverage={record['considered_action_count']}/"
            f"{record['legal_action_count']} "
            f"({record['action_coverage']:.1%}) | "
            f"visited={record['visited_action_count']}/"
            f"{record['legal_action_count']} "
            f"({record['visited_action_coverage']:.1%}) | "
            f"done={record['done']} | "
