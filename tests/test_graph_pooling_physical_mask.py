...
def _context_probe_model() -> GraphPolicyValueNetV2:
    ...
    model.global_projection = _SliceProjection(edge_mean_start, hidden_dim)
...
def test_active_non_switchable_bridge_changes_value():
    ...
    changed_branch_features[1, 0] = 9.0
...
def test_inactive_edge_features_do_not_change_policy_or_value():
    ...
    action_mask = torch.tensor([True, True, False])
    changed_branch_features = inputs["branch_features"].clone()
    changed_branch_features[1] = torch.tensor([10000.0, -10000.0])
    ...
    torch.testing.assert_close(original_logits, changed_logits, rtol=0.0, atol=0.0)
    torch.testing.assert_close(original_value, changed_value, rtol=0.0, atol=0.0)
...
def test_masked_edge_matches_physically_removed_edge_in_message_passing():
    ...
    hidden_dim = 6
    layer = ResidualEdgeMessagePassingV2(hidden_dim=hidden_dim, dropout=0.0)
    layer.eval()
    node_embeddings = torch.randn(4, hidden_dim)
    edge_embeddings = torch.randn(3, hidden_dim)
    edge_index = torch.tensor(
        [[0, 1, 2], [1, 2, 3]],
        dtype=torch.long,
    )
    ...
    torch.testing.assert_close(full_nodes, pruned_nodes, rtol=1e-5, atol=1e-6)
    torch.testing.assert_close(
