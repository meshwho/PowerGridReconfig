...
def _state_metadata(*, missing_field: str | None = None) -> np.ndarray:
    metadata = {
        "run_id": _RUN_ID,
...
def _write_fake_state(path, action_mask, *, missing_metadata_field: str | None = None):
    ...
    np.savez(
        path,
        bus_features=np.zeros((2, len(BUS_FEATURE_COLUMNS)), dtype=np.float32),
        ...
        metadata_json=_state_metadata(missing_field=missing_metadata_field),
    )
        ...
        [
            {
                ...
                "action_layout_fingerprint": action_layout_fingerprint(_ACTION_LAYOUT),
                "run_id": _RUN_ID,
                "iteration": _ITERATION,
                "episode_id": _EPISODE_ID,
                "scenario_id": 1,
...
def test_dataset_rejects_probability_on_masked_action(tmp_path):
    ...
    _write_examples_csv(examples_csv, state_path, {"1": 0.25, "2": 0.75})
    with pytest.raises(ValueError, match="masked"):
        GraphSelfPlayDataset(examples_csv=examples_csv, normalize_features=False)
...
def test_dataset_rejects_policy_actions_outside_action_space(tmp_path):
    ...
    _write_fake_state(state_path, [True, True, True])
    examples_csv = tmp_path / "examples.csv"
    _write_examples_csv(examples_csv, state_path, {"1": 0.25, "99": 0.75})
    with pytest.raises(ValueError, match="out of range"):
        GraphSelfPlayDataset(examples_csv=examples_csv, normalize_features=False)


@pytest.mark.parametrize("policy", [{"1": 0.8}, {"1": 1.2}])
...
def test_dataset_rejects_wrong_action_mask_length(tmp_path):
    ...
    examples_csv = tmp_path / "examples.csv"
    _write_examples_csv(examples_csv, state_path, {"0": 1.0})
    with pytest.raises(ValueError, match=r"action_mask must be 1D with 3 entries"):
        GraphSelfPlayDataset(examples_csv=examples_csv, normalize_features=False)


def test_dataset_rejects_incomplete_state_metadata(tmp_path):
    state_path = tmp_path / "state.npz"
    _write_fake_state(
        state_path,
        [True, True, True],
        missing_metadata_field="terminal_outcome_evidence",
    )
    examples_csv = tmp_path / "examples.csv"
    _write_examples_csv(examples_csv, state_path, {"0": 1.0})
