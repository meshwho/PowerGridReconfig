...
def test_segment_softmax_handles_amp_promoted_exp(monkeypatch) -> None:
    scores = torch.tensor([1.0, 2.0, -1.0, 3.0], dtype=torch.float16)
...
def test_graph_v2_forward_and_backward_with_cuda_amp() -> None:
    ...
    bus_features = torch.randn(8, 3, device=device)
    branch_features = torch.randn(6, 2, device=device)
    ...
    node_batch = torch.tensor([0, 0, 0, 0, 1, 1, 1, 1], device=device)
    edge_batch = torch.tensor([0, 0, 0, 1, 1, 1], device=device)
    action_mask = torch.ones(2, 4, dtype=torch.bool, device=device)
